...
app.post('/api/portfolio', (req, res) => {
  ...
  if (!userId || !model || !quantity || Number(quantity) < 1) {
    return res.status(400).json({ error: 'userId, model et quantity (>=1) sont requis.' });
  }
...
app.post('/api/listings', (req, res) => {
  ...
  if (!model || !price || !condition || !seller || Number(price) <= 0) {
    return res.status(400).json({ error: 'model, price (>0), condition et seller sont requis.' });
  }
